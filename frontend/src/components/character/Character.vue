...
import UpdateIcon from "@/components/icon/UpdateIcon.vue";
  ...
  <div>
    <div class="avatar cursor-pointer" @mouseover="isHover=true" @mouseout="isHover=false">
      <div class="w-60 h-100 rounded-2xl relative">
        ...
        <div v-if="canEdit && character.author.user_id === user.id" class="absolute right-0 top-67">
        ...
        <div class="absolute left-4 top-60 avatar">
          <div class="w-16 rounded-full ring-3 ring-white">
            <img :src="character.photo" alt="">
          </div>
        </div>
        <div class="absolute left-24 right-4 top-60 text-white font-bold line-clamp-1 break-all">
          {{ character.name }}
        </div>
        <div class="absolute left-5 right-4 top-80 text-white line-clamp-3 break-all">
          {{ character.profile }}
        </div>
      </div>
    </div>
